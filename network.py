...
class DeConv2d(nn.Module):
    def __init__(self, in_c, out_c,k_size,stride,padding=0, activate=True, In=False):
        ...
        self.deconv = nn.ConvTranspose2d(in_c, out_c, kernel_size=k_size, stride=stride, padding=padding, output_padding=0)
        self.In = nn.InstanceNorm2d(out_c, affine=True) if In else None
        self.relu = nn.ReLU(inplace=True) if activate else None
...
class SANet(nn.Module):
    ...
    '''
    SANet: Scale Aggregation Network for Accurate and Efficient Crowd Counting, ECCV 2018
    Source: https://github.com/JoshuaSXA/SANet-PyTorch/blob/master/model.py
    '''
    ...
    def __init__(self):
        ...
        self.out_layer = Conv2d(16, 1, 1, padding=0, In=False, activate=False)
        self.relu_final = nn.ReLU(inplace=True)  # Đảm bảo density map non-negative
    ...
    def forward(self, x):
        input_size = x.shape[2:]  # Save input (H, W)
        x = self.fme_module(x)
        x = self.dme_module(x)
        x = self.out_layer(x)
        x = self.relu_final(x)  # Apply ReLU to ensure non-negative density
        
        # Resize output to match input size (handle size mismatch from conv/deconv)
